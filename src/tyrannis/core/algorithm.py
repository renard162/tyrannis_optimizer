...
class AlgorithmBase(ABC):
    ...
    def pre_iteration(self, actual_iter: int) -> None:
        ...
        Random values that will be required by ``update_particle`` should
        preferably be generated during this method and stored in the respective
        particle's ``random_cache``. These values are then consumed by
        ``update_particle`` instead of generating new random values during the
        particle update.

        Pre-generating random values is particularly important when particle
        updates are executed in parallel. It prevents different worker processes
        from independently advancing deterministic random number generators from
        equivalent states, which can result in identical or otherwise correlated
        random values being generated by different workers.

        The ``random_cache`` is intended only as temporary storage for random
        values required by the current iteration. It is automatically cleared
        when ``update_population`` is executed after the particle update phase.
    ...
    def update_particle(self, identifier: str) -> ParticleBase:
        ...
        Random values required by the particle update should preferably be
        obtained from the particle's ``random_cache``. These values should have
        been generated previously by ``pre_iteration`` rather than being
        generated during this method.

        Avoiding random number generation during the particle update is
        particularly important when updates are executed in parallel. Worker
        processes may otherwise initialize or advance deterministic random
        number generators from equivalent states, potentially causing different
        particles to receive identical random values. Pre-generating the random
        values in ``pre_iteration`` ensures that the values are generated in the
        main algorithm process before parallel execution begins.

        The ``random_cache`` contains temporary values intended only for the
        current particle update phase. Its contents are cleared automatically
        when ``update_population`` is executed.
