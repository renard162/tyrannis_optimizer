...
def create_processor(seed: int = SEED) -> ThreadsPool:
    algorithm = PSO()
...
def run_processor(seed: int = SEED) -> ThreadsPool:
...
def test_processors_pool_creates_threads_executor() -> None:
    ...
    assert isinstance(executor, ThreadsPool)
