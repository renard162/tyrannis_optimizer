...
def test_processor_init() -> None:
    ...
    assert not processor._running.is_set()
    assert isinstance(processor.messages, Queue)
    assert isinstance(processor.outgoing_queue, Queue)
...
def test_processor_process_buffer() -> None:
    ...
    assert [processor.messages.get(), processor.messages.get()] == [
        "message-1",
        "message-2",
    ]
    assert processor._receive_buffer == ""
...
def test_driver_init() -> None:
    ...
    assert driver._island_ids == {FIRST_ISLAND, SECOND_ISLAND}
...
def test_driver_extract_messages_preserves_incomplete_frame() -> None:
    ...
    assert driver._extract_messages(connection) == ["partial"]
    assert driver._receive_buffers[connection] == ""
...
def test_driver_receive_message_routes_messages() -> None:
    ...
    with (
        patch.object(driver, "_receive_data", return_value=True),
        patch.object(
            driver,
            "_extract_messages",
        ...
        driver._receive_message(connection, FIRST_ISLAND)
...
def test_driver_send_pending_messages_on_error() -> None:
    ...
    with patch.object(driver, "_close_connection") as close_connection:
        driver._send_pending_messages()

    assert driver.outgoing_queues[FIRST_ISLAND].get() == "message"
    close_connection.assert_called_once_with(connection, FIRST_ISLAND)
...
def test_driver_send_stop_to_all() -> None:
    ...
    first.sendall.assert_called_once_with(driver.STOP.encode())
    second.sendall.assert_called_once_with(driver.STOP.encode())


def test_driver_close_connection() -> None:
    driver = create_driver()
    connection = Mock()
    selector = Mock()
    ...
    driver._close_connection(connection, FIRST_ISLAND)
